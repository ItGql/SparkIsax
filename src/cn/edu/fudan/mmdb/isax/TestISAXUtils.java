package cn.edu.fudan.mmdb.isax;
...
import cn.edu.fudan.mmdb.sax.SAXException;
...
public class TestISAXUtils {
	...
	public void testTStoISAXdecomp() {
		...
		// now check to see that the symbos in the iSAX sequence match whats
		// expected.
	...
	public void testISAXSequenceSplitTest() {
		...
		try {
			seq_out = ISAXUtils.CreateiSAXSequenceBasedOnCardinality(ts, A); // (
																				// ts,
																				// 4,
																				// 4
																				// );
	...
	public void testISAXSequenceSplitTest_1() {
		...
		try {
			seq_out_0 = ISAXUtils.CreateiSAXSequenceBasedOnCardinality(ts, A); // (
																				// ts,
																				// 4,
																				// 4
																				// );
		...
		try {
			seq_out_1 = ISAXUtils.CreateiSAXSequenceBasedOnCardinality(ts, A2); // (
																				// ts,
																				// 4,
																				// 4
																				// );
...
//		System.out.println("\nbased on: " + A.getStringRepresentation());
	...
	public void testISAXSequenceSplitTest_2() {
		...
		/*
		 * Sequence A2 = new Sequence( 16 ); // T, 4, 8 A2.getSymbols().add(new
		 * Symbol( 3, 4 ) ); A2.getSymbols().add(new Symbol( 3, 4 ) );
		 * A2.getSymbols().add(new Symbol( 1, 4 ) ); A2.getSymbols().add(new
		 * Symbol( 0, 8 ) );
		 */
		// A2.getSymbols().get(3).
		...
		try {
			seq_out_0 = ISAXUtils.CreateiSAXSequenceBasedOnCardinality(ts, arCards); // (
																						// ts,
																						// 4,
																						// 4
																						// );
	 ...
	 * 
	 * --- if we always find our cut points from paa_val, theres no way for it
	 * to be out of the ranges unless its at either end out of range.
	 * 
	 * -------- is that right?
	 */
	@Test
