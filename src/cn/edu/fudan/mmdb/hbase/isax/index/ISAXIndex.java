...
/**
 * - how do we want to reference a table now? - need to look at ways to set it
 * at query time yet not propogate the string everywhere
 *
 * - does this run as a service that pulls conf files each query?
 * 
 * - need to think about things in terms of client lib - and then in terms as
 * running tornado/jetty service, conf vs command params
 * 
 * - what is base case?
 * 
 * @author jpatterson
...
public class ISAXIndex implements Iterable<NodePersisted> {
	...
	final public static String TABLE_NAME = "isaxGqlVersion2.0";
	final public static String TABLE_NAME_NEW = "SuperSAX_33180_0";
	final public static int INDEX_NUMBER = 200000;
	...
	public InternalNodePersisted root_node = null;
	// this is public cause imlazy during dev
	// need to have a non-static index specific value here, could be working
	// with multiple
	private String hbase_table_name = "";

	/*
	 * CTOR ------------- need to work out how these hardcoded params are going
	 * to be loaded dynamically depending on table
	 */
	public ISAXIndex() {
		// this.hbase_table_name = hbase_table_name;
		// we could set these params in a conf.xml file
		/*
		 * params = new IndexHashParams(); params.base_card = 4; params.d = 1;
		 * params.isax_word_length = 4; params.orig_ts_len = 8; params.threshold
		 * = 100;
		 */
		// Sequence s = new Sequence( 8 ); // root node seqeunce, needs nothing
		// more than a word len

		// Sequence s = new Sequence( params.orig_ts_len ); // root node
		// seqeunce, needs nothing more than a word len
		// this.LoadRoot();
	...
	private void LoadRoot() throws Exception {
		...
		try {
			node_bytes = HBaseUtils.Get(ROOT_NODE_KEY, this.hbase_table_name, ISAXIndex.COLUMN_FAMILY_NAME,
		...
		if (null == node_bytes) {
			// insert root
			/*
			 * Sequence root_key = new Sequence( params.orig_ts_len ); // root
			 * node seqeunce, needs nothing more than a word len this.root_node
			 * = new InternalNodePersisted( root_key, params, NodeType.ROOT );
			 * byte[] bytes_node = this.root_node.getBytes(); try {
			 * HBaseUtils.Put( ROOT_NODE_KEY, this.hbase_table_name, "node",
			 * "store", bytes_node ); } catch (IOException e)
			...
			// System.out.println( "\n\nISAXIndex > Found Root! > " +
			// node_bytes.length + " bytes" );
	...
	public static boolean CreateNewIndex(String table_name, String strBaseCard, String strDimPerSplit,
			String base_word_len, String base_ts_sample_size, String split_threshold) {
		/*
		 * params = new IndexHashParams(); params.base_card = 4; params.d = 1;
		 * params.isax_word_length = 4; params.orig_ts_len = 8; params.threshold
		 * = 100;
	...
	public static boolean CreateNewIndex(String table_name, IndexHashParams p) {
		...
		// its key to have the root since this is where the core params live
		// that define the resolution of the table

		// root node seqeunce,needs nothing more than a word len
		Sequence root_key = new Sequence(p.orig_ts_len);
		...
		try {
			HBaseUtils.Put(ROOT_NODE_KEY, table_name, ISAXIndex.COLUMN_FAMILY_NAME, ISAXIndex.STORE_COL_NAME,
					bytes_node);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static void DebugIndex(String table_name) {
		...
		// at some point we want to be able to iterate through all of the nodes
		// in the index
	}
	...
	/*
	 * This is the basic timeseries insert function for the hbase-based isax
	 * index
	 */
	...
	public void InsertSequence(TimeseriesInstance ts_inst) throws HashTreeException {
		...
		// create a root node
		// InternalNode root_node = new InternalNode( new Sequence(
		// this.params.orig_ts_len ), this.params, NodeType.ROOT );
		if (null == this.root_node) {
	...
	/*
	 * public boolean InsertDNASequence( String sequence, String source_name,
	 * int offset ) { Timeseries ts_dna = null; if (
	 * this.root_node.params.orig_ts_len != sequence.length() ) { return false;
	 * } try { ts_dna = ISAXUtils.CreateTimeseriesFromDNA( sequence ); } catch
	 * (TSException e) { e.printStackTrace(); } this.InsertSequence(ts_dna,
	 * source_name, offset ); return true; }
	 */
	@Override
