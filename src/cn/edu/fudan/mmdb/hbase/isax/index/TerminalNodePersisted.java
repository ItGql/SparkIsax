...
public class TerminalNodePersisted extends NodePersisted {
	...
	public TerminalNodePersisted(Sequence seq_key, IndexHashParams params,ArrayList<Timeseries> arts) {
		super();
		this.setType(NodeType.TERMINAL);
		this.key = seq_key;
		this.params = params;
		for(Timeseries x:arts){
			arInstances.put(x.toString(),new TimeseriesInstance((x)));
		}
	}
	/**
	...
	public void Insert(TimeseriesInstance ts_inst) {
		...
		try {
			// lets get our SAX word based on the params of this node and its
			// key
		...
		if (this.key.getIndexHash().equals(isax_hash)) {
			...
			// this.DebugInstances();
			// System.out.println( "TerminalNode > Inserted(instances:" +
			// this.arInstances.size() + ") > " + isax_hash + " @ " +
			// this.key.getStringRepresentation() + ", occurrences: " +
			// this.arInstances.get( ts_inst.getTS().toString()
			// ).getOccurences().size() ) ;
			// ------ ok, either way at this point, the data structures are
			// updated. ---- update hbase -----
			byte[] b = this.getBytes();
			try {
				HBaseUtils.Put(this.key.getIndexHash(), this.GetTableName(), ISAXIndex.COLUMN_FAMILY_NAME,
						ISAXIndex.STORE_COL_NAME, b);
			} catch (IOException e) {
				e.printStackTrace();
			}
			// System.out.println( "\n\nTerminalNodePersisted > Structures
			// Updated > " + b.length + " bytes Written to HBase!" );
		} else {
	...
	public TimeseriesInstance ApproxSearch(Timeseries ts) {
		Sequence ts_isax = null;
		// ArrayList<Integer> arCards =
		// IndexHashParams.generateChildCardinality( this.key );
		...
		// System.out.println( "Searching For > Seq: " +
		...
		String isax_hash = ts_isax.getIndexHash(); // this.params.createMaskedBitSequence(isax);
		// termination check, AKA "is this me?"
		// System.out.println( "key( " + this.key.getIndexHash() + " ) ==
		// search( " + isax_hash + " ) ?" );
		if (this.key.getIndexHash().equals(isax_hash)) {
			// System.out.println( "found > key > " + isax_hash + ", looking for
			// exact match" );
			return this.SearchLocalInstancesForClosestMatch(ts, isax_hash);
			/*
			 * if ( this.arInstances.containsKey(ts.toString() ) ) { return
			 * this.arInstances.get(ts.toString() ); }
			 */
		} // if
		return null;
	}
	...
	public TimeseriesInstance SearchLocalInstancesForClosestMatch(Timeseries ts, String isax_hash) {
		...
		while (itr.hasNext()) {
			...
			if (null == closest_so_far) {
				// System.out.println( "TerminalNode > Debug > Null: " + strKey
				// + ", count: " + this.arInstances.size() );
	...
	/*
	 * @Override public void LoadFromStore( String sax_hash_key ) { this.key =
	 * new Sequence( 0 ); this.key.parseFromIndexHash( sax_hash_key ); // CALL
	 * to hbase here, pull bytes @ key: sax_hash_key byte[] node_bytes = null;
	 * //---------- wire in HBASE // deserialize bytes into Node
	 * this.deserialize( node_bytes ); // add orig length into sax key
	 * this.key.setOrigLength( this.params.orig_ts_len ); }
	 * 
	 * @Override public void WriteToStore() { // hash key to get HBase key
	 * String key = this.key.getIndexHash(); // serialize node into bytes byte[]
	 * node_bytes = this.getBytes(); // PUT into hbase // ---------- wire in
	 * HBASE }
	 */
	// 相当于反序列化
	...
	public void deserialize(byte[] src_bytes) {
		...
		try {
			...
			in.close();
			// System.out.println( "TerminalNodePersisted > SerDe >
			// TerminalNodePersisted > num instances : " +
