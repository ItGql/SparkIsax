...
public final class Timeseries implements Iterable<TPoint>, Cloneable, Serializable {
   ...
   */
  //offset->offset+tslen的时间序列


  public Timeseries(double[] values,int offset,int tslen) throws TSException {
    meter_Id=String.valueOf((long)values[0]);
    for(int i = offset; i < offset+tslen; i++) {
        this.series.add(new TPoint(values[i], i-offset));
    }
  }
  public Timeseries(double[] values) throws TSException {
